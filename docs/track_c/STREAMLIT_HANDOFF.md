...
9. **입력 전체를 하나의 질문으로 검색한다.** 여러 독립적인 질문을 한 번에 넣으면 일부 근거가 검색되지 않는다. 팀 합의(2026-09-01)에 따라 나눠서 답하지 않고 `needs_clarification`으로 하나만 물어봐 달라고 되돌려준다. 판정은 확실한 신호만 본다 — 물음표 둘 이상, 또는 쉼표로 세 가지 이상 나열. `창덕궁과 덕수궁은 같은 궁궐이야?`처럼 대상이 둘이어도 묻는 것이 하나면 막지 않는다. **멀쩡한 질문을 막는 쪽이 답을 못 주는 것보다 나쁘다.**
10. **근거 선택이 문서당 1개로 자른다.** Track A 실험 결론(PR #23)은 검색 단계에 문서당 제한을 두지 않는 것인데, `pick_documents()`가 그 뒤에서 문서당 하나만 남긴다. 복합 질문에 같은 문서의 여러 청크가 필요할 때 걸림돌이 된다. 임시 생성기의 규칙이라 생성 컴포넌트가 붙으면 사라지지만, 그 전까지는 이 상태다.
...
### 하이브리드 검색은 선택 사항이다 (PR #19)

Pinecone은 팀 공용이지만 **BM25 인덱스는 각자 로컬에서 만들어야 한다.** 만들지 않아도 화면은 그대로 돌아간다. 인덱스 파일이 있으면 하이브리드, 없으면 의미 검색 단독으로 자동 전환된다. 어느 쪽으로 검색했는지는 공정 견학 탭 2단계에 그대로 표시된다.

만들려면 청킹 완료 JSONL이 필요하다. 저장소에는 없고 Drive에서 받아야 한다.
...
| `경복궁에 대해 알려줘` | **검색이 실패하던 사례.** BM25 인덱스가 있으면 순위가 달라진다 |
