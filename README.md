...
> 문서 기준일: 2026-09-02
>
> 기준 코드: PR #25와 PR #28까지 반영된 `main` (`744b12c`)
...
| 화면 | 기본 통합 완료 | 질문, 답변 과정, 공정 견학, 평가와 전체 데이터 기반 탐험 화면 제공 |
...
- 발표자료의 프로젝트명을 `개인 맞춤형 AI 문화유산 가이드`로 통일
- LangGraph 슬라이드의 결론을 `개선안의 정확도 향상 확인, 지연과 일정 때문에 3차 미반영`으로 수정
- PR #27이 3차 프로젝트에 반영된 기능처럼 보이지 않도록 Prompt 설명과 시연 화면 확인
...
| `1팀_RAG기반_LLM(개인_맞춤형_AI_문화유산_가이드).zip` | 코드, `requirements.txt`, README | PR #22 병합 후 실행 검증·패키징 |
